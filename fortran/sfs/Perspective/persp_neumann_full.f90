...
do while (error> tol)
   ...
   do k = 2,N
      ...
      L = I(1,k)*sqrt(f**2*(Dx**2+Dy**2) + (Ax(Ix)*x(1)+Ay(Iy)*y(k))**2 + f**2/(x(1)**2+y(k)**2+f**2)) - f/sqrt(x(1)**2+y(k)**2+f**2)
   ...
   do k=2,N
      ...
      L = I(N+1,k)*sqrt(f**2*(Dx**2+Dy**2) + (Ax(Ix)*x(N+1)+Ay(Iy)*y(k))**2 + f**2/(x(N+1)**2+y(k)**2+f**2)) - f/sqrt(x(N+1)**2+y(k)**2+f**2)
